...
from tf2marl.agents.matd3 import MATD3Agent
...
def test_save_load():
    ...
    env = IdentityEnv(5, 2)
    agents = [MATD3Agent(env.observation_space, env.action_space, idx, batch_size=32, buff_size=10000, lr=0.01, num_layer=2,
    ...
    load_agents = [MATD3Agent(env.observation_space, env.action_space, idx, batch_size=32, buff_size=10000, lr=0.01, num_layer=2,
                          num_units=32, gamma=0.9, tau=0.01, prioritized_replay=True,
                          max_step=5000) for idx in range(2)]
    ...
    assert check_for_equal_arrays(load_agents[0].critic_1.model.get_weights(), agents[0].critic_1.model.get_weights())
    assert check_for_equal_arrays(load_agents[1].critic_1.model.get_weights(), agents[1].critic_1.model.get_weights())
    assert check_for_equal_arrays(load_agents[0].critic_2.model.get_weights(), agents[0].critic_2.model.get_weights())
    assert check_for_equal_arrays(load_agents[1].critic_2.model.get_weights(), agents[1].critic_2.model.get_weights())

    assert check_for_equal_arrays(load_agents[0].critic_1_target.model.get_weights(), agents[0].critic_1_target.model.get_weights())
    assert check_for_equal_arrays(load_agents[1].critic_1_target.model.get_weights(), agents[1].critic_1_target.model.get_weights())
    assert check_for_equal_arrays(load_agents[0].critic_2_target.model.get_weights(), agents[0].critic_2_target.model.get_weights())
    assert check_for_equal_arrays(load_agents[1].critic_2_target.model.get_weights(), agents[1].critic_2_target.model.get_weights())
...
def test_identity_discrete(agent_number, prioritized_replay):
    ...
    agents = [MATD3Agent(env.observation_space, env.action_space, idx, batch_size=32, buff_size=10000, lr=lr, num_layer=2,
...
def test_identity_continuous(agent_number, prioritized_replay):
    ...
    lr = 0.01 if not prioritized_replay else 0.02
    agents = [MATD3Agent(env.observation_space, env.action_space, idx, batch_size=100, buff_size=10000, lr=lr, num_layer=2,
                          num_units=32, gamma=0.9, tau=0.01, prioritized_replay=prioritized_replay,
                          max_step=5000, _run=FakeRun()) for idx in range(agent_number)]
